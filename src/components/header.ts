...
import { getUserTokenLocalStorage, removeUserLocalStorage } from '../utils/auth';
...
class Header {
  ...
  private createButtons(): HTMLElement {
    const buttons: HTMLElement = this.template.createElement('div', `${this.mainClass}__buttons`);
  ...
  private createWrap(): HTMLElement {
    const wrap: HTMLElement = this.template.createElement('div', `${this.mainClass}__wrap`);
    return wrap;
  }

  private createListMenu(): HTMLElement {
    const list: HTMLElement = this.template.createElement('ul', `${this.mainClass}__list`);
  ...
  private createLinksMenu(): HTMLElement[] {
    ...
    nameLinksMenu.forEach((nameLink) => {
      const listItem: HTMLElement = this.template.createElement('li', `${this.mainClass}__list-item`);
  ...
  private createLink(name: string): HTMLAnchorElement {
    const link: HTMLAnchorElement = this.template.createLink(`${this.mainClass}__link`, '/', name);
  ...
  private createBorderLink(): HTMLElement {
    const borderLink: HTMLElement = this.template.createElement('span', `${this.mainClass}__link-border`);
    return borderLink;
  }
