...
class StartTrainingPage {
  ...
  public async draw() {
    ...
    const setttingsModal: HTMLElement = this.template.createElement('div', 'settingsModal');
    ...
    setttingsModal.addEventListener('click', (e) => {
      ...
      if (target.classList.contains('settingsModal')) setttingsModal.classList.add('invisible');
    });
    ...
    this.currentExerciseIndex = 0;
    const curExercise = new ExerciseBlock(this.exerciseArray[this.currentExerciseIndex]);
    mainPageElement.append(curExercise.draw());
    if (this.currentExerciseIndex === 0 && this.exerciseArray[this.currentExerciseIndex].quantity.toLowerCase().includes('x')) {
      ...
      setTimeout(() => {
        const duration = this.getExerciseDuration(this.exerciseArray[this.currentExerciseIndex]);
    ...
    mainPageElement.addEventListener('click', (e) => {
      ...
      if (target.classList.contains('exercise-block__button-pause')) {
        ...
        const modal = new PauseModal(this.exerciseArray[this.currentExerciseIndex]);
  ...
  private async showRestModal() {
    const pageContent = <HTMLElement>document.querySelector('.startTraining-page');
    pageContent.innerHTML = '';
    pageContent.append(await this.takeARest.draw(this.currentExerciseIndex, this.exerciseArray));
    const userId1: string | undefined = getUserIdLocalStorage();
    if (!userId1) {
      return;
    }
  ...
  private async showCongrats(counter: number, time: number) {
    ...
    const completeBtn = <HTMLAnchorElement>document.querySelector('.congrats__button-complete');
  ...
  setTimeCounter(duration: number): void {
    ...
    this.interval = setInterval(() => {
      const quantity = <HTMLElement>document.querySelector('.exercise-block__current-quantity');
      if (quantity) {
        ...
        const widthEl = <HTMLElement>document.querySelector('.exercise-block__time-bar');
        if (widthEl) {
          const maxWidth = widthEl.clientWidth;
          const track = <HTMLElement>document.querySelector('.exercise-block__track');
  ...
  getExerciseDuration(exercise: IExercise): number {
    ...
    const duration: number = durMins !== 0 ? durMins * 60 + durSeconds : durSeconds;
    return duration;
  }

  createThreeCount(): void {
    const threeCounter: HTMLElement = this.template.createElement('div', 'counter-three');
    const counter = <HTMLElement>document.querySelector('.exercise-block__counter');
  ...
  closeModal() {
    const backLayer = <HTMLElement>document.querySelector('.pause-modal__backlayer');
    backLayer.remove();
  }

  private createSettingsModal(id: string): HTMLElement {
    const wrapper: HTMLElement = this.template.createElement('div', 'modal-addNewComplex__addName');
    wrapper.classList.add('settings__modal');
    const title: HTMLElement = this.template.createElement('h2', 'modal-addNewComplex__title', 'Workout Settings');
    const restTimeBlock = this.createSettingsTimeBlock('Rest time, seconds', '20', mug, false, 'settings__rest');
    const loadBlock = this.createSettingsTimeBlock('Number of Workouts', '3', lightning2, true, 'settings_load');
    const doneWrap: HTMLElement = this.template.createElement('div', 'settings__button-wrap');
    const done: HTMLButtonElement = this.template.createBtn('modal-addNewComplex__create', 'Done');
    doneWrap.append(done);
      ...
      const restTimeInput = document.querySelector('.settings__rest') as HTMLInputElement;
      const loadInput = document.querySelector('.settings_load') as HTMLInputElement;
  ...
  private createSettingsTimeBlock(title1: string, content: string, svg: string, flag: boolean, addClass: string): HTMLElement {
    const restTimeBlock: HTMLElement = this.template.createElement('div', 'settingsMiniBlock');
    const square: HTMLElement = this.template.createElement('div', 'settings-square');
    square.innerHTML = svg;
    const textWrapper = this.template.createElement('div', 'settings__text-wrapper');
    const textTitle: HTMLElement = this.template.createElement('div', 'settings__text-title', title1);
